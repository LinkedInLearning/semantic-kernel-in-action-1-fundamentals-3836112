...
public class ResearcherProject
{
    ...
    string topicOfResearch)
  {
    string summarizeWebSearch = @"
            ...
            {{#each webSearchResults}}
                ...
                {{set ""searchResults"" (concat searchResults ""Snippet: "" this.Snippet  ""\n"")}}
    ...
    // This HB prompt iterates through the web search results and summarizes them,
    // providing an enriched search result with the summary of each of the web search results.
    // and it generates a report.
    string summarizeWebSearchAndGenerateReport = @"
            {{set ""searchResults"" (concat  ""---\n"" "" Search Results \n Topic: "" topicOfResearch ""\n---\n"")}}

            {{#each webSearchResults}}
                {{set ""scrapedWebResult"" (WebScrapperPlugin-Scrape this.Url)}}
                {{set ""summary"" (WebSearchSummarizator-GenerateWebSearchSummary name=this.Name snippet=this.Snippet scrapedContent=scrapedWebResult)}}
                {{set ""searchResults"" (concat searchResults ""URL: "" this.Url  ""\n"")}}
                {{set ""searchResults"" (concat searchResults ""Name: "" this.Name  ""\n"")}}
                {{set ""searchResults"" (concat searchResults ""Summary: "" summary  ""\n"")}}
                {{set ""searchResults"" (concat searchResults ""---\n"")}}  
            ...
            {{set ""reportForResearchTopic"" (WebSearchSummarizator-GenerateResearchReport topicofresearch=topicOfResearch searchresults=searchResults)}}
    ...
    var customHandlebarsPromptResult = await kernel.InvokeAsync(
                ...
                new() {
                        { "webSearchResults", webSearchResults.Take(3) },
  ...
  private static async Task AddWebSearchAnalysisPlugin(Kernel kernel)
  {
    ...
    var kernelFunctionResearchReportMaker = KernelFunctionFactory.CreateFromPrompt(
            "You are an expert report maker." +
            "Your goal is to make a report on a concrete topic and elaborate a report on the web findings summaries provided. " +
            "You must produce a concise summary report and elaborate the top 5 to 10 most important and relevant points. " +
            "You will receive, for each search result:" +
            " URL - URL of the web result." +
            " Name - Name of the web result." +
            " Summary - A concise summary of the web result including some of the main points." +
            "With this you will analyze those results and provide a precise and concise report on the topic, made of:" +
            "1. A Title" +
            "2. A Summary" +
            "3. Relevant points" +
            "4. Reference URLs" +
            ...
            "After the summary, the relevant points should be the top 5-10 most important and relevant points stated by the search results." +
            "The relevant points must be preceded by 'More relevant points:'." +
            ...
            "For the summary, use 500 to 1000 words." +
